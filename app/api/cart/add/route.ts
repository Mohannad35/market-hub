...
import { addToCartSchema } from "@/lib/validation/cart-schema";
...
async function POST_handler(request: NextRequest): Promise<NextResponse<Cart>> {
  ...
  const { success, data, error } = addToCartSchema.safeParse(body);
    ...
    data: {
      cartItems: {
        upsert: {
          ...
          create: { productId, quantity, priceAfter: product.price },
          update: { quantity, priceAfter: product.price },
        },
      },
    },
  });
